...
import { cn, getRegistryBaseUrl, getRegistryItemUrl } from "@/lib/utils";
import { Button } from "@/registry/new-york/ui/button";

function buildV0Url(name: string, base?: string) {
  const registryItemUrl = getRegistryItemUrl(name, base);
  if (!registryItemUrl) return undefined;
  const v0Url = new URL("https://v0.dev/chat/api/open");
  v0Url.searchParams.set("url", registryItemUrl);
  return v0Url.toString();
}
